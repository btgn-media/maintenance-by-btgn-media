...
* **Session bypass:** Define your own URL parameter per project. Visiting `https://example.com/?your-parameter` sets a signed session cookie, so that visitor sees the full website until the browser is closed. Great for sharing a preview link with a client, no login required.
* **Two display modes:**
    1. Custom HTML & CSS, which loads the site's styles, so utility frameworks like Automatic.css (ACSS) work directly.
    2. Any existing page, built with any builder (Etch, Bricks, Elementor, Gutenberg, etc.), with optional switches to hide the theme header and/or footer.
